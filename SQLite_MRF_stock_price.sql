...
select 
date,
volume from MRFNS where volume = 0;
...
open, 
close, 
close- open as day_hike FROM MRFNS order by day_hike desc limit 10;
...
open,
close, 
(close - open)*100/open as percentage_increse_stock_price, 
max(volume) from MRFNS;
...
open,
close, 
(close - open)*100/open as percentage_price from MRFNS;
...
adj_close, 
volume, 
close- open as Today_stock_price from MRFNS;
...
volume, 
open,
open- LAG(open) over (order by date) as Diff_to_last_day FROM MRFNS;
...
create table return as select 
date,
open from MRFNS where date in ("2002-07-01","2021-09-29");
--select *from return;

select 
open - LAG(open) over (order by date) as return_in_RS, 
date- Lag(Date) over (order by date) as in_years FROM return;
