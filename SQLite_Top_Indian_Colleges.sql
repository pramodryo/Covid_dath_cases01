...
-- find Repeated entrees of college data
...
SELECT state, count(college_name) as total_college 
FROM College_data 
GROUP by state 
order by total_college desc;


--Count all college type presented in India
SELECT stream, count( college_name) as Total_college
FROM College_data
GROUP BY stream 
order by Total_college DESC;
...
SELECT college_name, state, rating 
FROM College_data 
...
SELECT college_name, state, stream,rating 
FROM College_data
where rating > 8.5 and academic > 8.5 and accommodation > 8.5 AND faculty > 8.5 AND infrastructure > 8.5 and placement > 8.5 and social_life > 8.5;


-- Find the Best Engineering college in maharastra 
SELECT college_name, ug_fee, pg_fee, rating,placement 
FROM College_data
where stream = "Engineering" and state = "Maharashtra" 
